...
if($_SERVER['REQUEST_METHOD'] == 'POST') {
    if($_SESSION['loggedUserId'] == $_GET['userId']) {
        ...
        $newPassword = !empty($_POST['new_password']) ? $conn->real_escape_string(trim($_POST['new_password'])) : '';
        $newPassword2 = !empty($_POST['new_password2']) ? trim($_POST['new_password2']) : '';
        $oldPassword = !empty($_POST['old_password']) ? $conn->real_escape_string(trim($_POST['old_password'])) : '';
        $newFullName = !empty($_POST['new_full_name']) ? $conn->real_escape_string(trim($_POST['new_full_name'])) : '';
        ...
        elseif((strlen($newPassword) == 0 && strlen($newPassword2) == 0) && strlen($newFullName) > 0) {
